...
% 23 10 2017 : Perrine add pcjoel as a parameter (max number of cell to divide (was
% set 0.18, said 0.25 in the presentation). Warning : Gar was set to 0???
...
% Create blood vessel for various traversal geometry
...
gar = [0 0 0 0 0 0]/24; % je considere que l'on a un arret du cycle de 10 jours quelque soit la dose.10 jours est un choix arbitraire. l'important est d'avoir un temps > au delais entre fractions. SCH
gar_days = interp1(d_ar,gar,fracD,'pchip');
% Number of days simulated
%sim_days = numD*delT*1.2;
A=mod(numD,2);
B=mod(numD,3);
C=mod(numD,5);
if (delT==1) % simule 5 seances par semaine
    sim_days=floor(numD/5)*7+C ;   
end
if delT==2 %simule 3 seances par semaine
    if B==0
       sim_days=(numD/3)*7;
    end
    if B==1 
        sim_days=(floor(numD/3)*7+1);
    end    
    if B==2
       sim_days=(floor(numD/3)*7+3);
    end
end
if delT==3 % simule 2 seances par semaine
       sim_days=floor(numD/2)*7+A;
end
sim_days=sim_days+1;    % prise en compte de la situation initiale t0 avant debut ttt qui compte pour 1
%disp(sim_days);
%disp(B);
Tcycle = 1; % cell cycle time in day
timer_day = 0; % Keep track of time in days
timer_rad = delT-1; % PERRINE: WARNING: start at delT-1 instead of 0: ortherwise first week would have a different protocol; % keep track of interval between radiation exposure
dose_cnt = 0;
...
%        growth
        for i_layer = 1:cell_layer
            ...
            max_div = int16(pcJoel*sum(layer_index>0)); % # of new cells should be limited to available space and number of cells that can divide
            %max_div = sum(layer_index>0);
            %disp(max_div);
            ...
            % only keep max_div fill gap randomly to make sure division onlyHypoxia_model(field_size,fracD,numD,delT,RTres,O2_th,leak_death,extra_days,O2_option,EC_death_option,cell_layer,parralel,alpha,beta,alv,bev,blood_density);
            % multiplies by 2
...
%           division)
 ...
 %            diptruesize(600)
    ...
    if and(and(mod(i_t,7)<6,mod(i_t,7)>0),and(timer_rad>=delT,dose_cnt<numD)) % Time to irradiate, except on day 6 and 7 (weekend)
       mydays='LMmJVSD';
       % disp(mydays(mod(i_t,7)));
        ...
        dose_cnt = dose_cnt + 1;
        timer_rad = 0; % PERRINE: WARNING: start at delT-1 instead of 0: ortherwise first week would have a different protocol
