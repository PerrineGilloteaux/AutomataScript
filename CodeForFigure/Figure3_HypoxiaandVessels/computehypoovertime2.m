function[ hypoxi_level_mean, hypoxi_level_std, num_t,cell_num,o2_lev]=computehypoovertime2(fracD,numD,delT,O2_th,numsamples,lf)
load(['Results\cellwithstacksm1m2_',num2str(fracD),'nbdose',num2str(numD),'delT',num2str(delT),'lf_',num2str(lf),'_run1.mat']);
num_t=size(o2_stack2,3); % # of time points
hypoxi_level = zeros(num_t,numsamples);
for i=1:numsamples
    load(['Results\cellwithstacksm1m2_',num2str(fracD),'nbdose',num2str(numD),'delT',num2str(delT),'lf_',num2str(lf),'_run',num2str(i),'.mat']);
    T_stack=T_stack2;
    o2_stack=o2_stack2;
    cell_num=cell_num2;
    o2_lev=o2_lev2;
