...
% November 2014
% Berkeley Lab
% +Perrine Paul-Gillteaux SFR F Bonamy
...
% Version 3Dsphere: 3D simultaion heavy in memory , just for control growth
% model
function [cell_stack,o2_stack,cell_num,o2_lev] = Hypoxia_model_3Dsphere(field_size,fracD,numD,delT,RTres,O2_th,leak_death,extra_days,O2_option,EC_death_option,cell_layer,parralel,waitfor)
...
cell_array = ones(field_size,field_size,field_size); % the tissue - 1 represents a normal cell
tumor_mask = newim(field_size,field_size,field_size);
tumor_mask(round(end/2),round(end/2),round(end/2)) = 1;
...
rand_array = rand(field_size, field_size,field_size);
...
dose_cnt = 0;
num_step = fix(sim_days/Tcycle) + extra_days+waitfor; % step at each cell cycle time (easier this way) - Add extra days to see if tumor grows back
if parralel==0
...
%             diptruesize(600)
    ...
    if and(i_t>waitfor,and(and(mod(i_t,7)<6,mod(i_t,7)>0),and(timer_rad>=delT,dose_cnt<numD))) % Time to irradiate, except on day 6 and 7 (weekend)
       mydays='LMmJVSD';
